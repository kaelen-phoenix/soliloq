...
- [ ] 8.2 Recorrido manual logueado: conmutar talento↔creador (acento del encabezado y del ítem activo cambia; botón nombra el destino); grilla del buscador con red lenta (sin salto, placeholder); `prefers-reduced-motion` en el SO (sin animaciones, nada oculto); etiquetas de familia de oficio y mensajes de estado sin cambios de color. **Pendiente: no hay `.env.local` en el entorno de trabajo, la app no levanta ahí. Lo hace el usuario en el deploy de prod (sesión real).**
- [~] 8.3 PR #11 mergeado a `main` (squash, `255ab05`); CI en verde; deploy de Vercel a producción en verde (`commits/main/status` → `success`). Smoke-test anónimo en `https://yalope.com`: `/` y `/talentos` responden `307 → /ingresar`, `/ingresar` `200`, sin 500. Pendiente: verificación logueada y Lighthouse/CLS.
