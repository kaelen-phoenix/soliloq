  ...
  return (
    <section className="flex flex-col gap-3">
      <ul className="flex flex-col gap-3">
        {metricas.map((m) => (
          <li key={m.rol_id} className="rounded-2xl border border-borde bg-superficie p-4">
            <FilaRol metrica={m} />
          </li>
        ))}
      </ul>
      ...
      <ul className="flex flex-wrap gap-x-4 gap-y-1.5 px-1">
        {SEGMENTOS.map((s) => (
          <li key={s.clave} className="flex items-center gap-1.5 text-2xs text-texto-tenue">
  ...
  return (
    <>
      <div className="flex items-baseline justify-between gap-3">
        <p className="min-w-0 truncate text-base font-medium text-texto">{m.rol_nombre}</p>
        ...
        <>
          ...
          <ul className="mt-2.5 flex flex-wrap gap-x-3.5 gap-y-1">
            {SEGMENTOS.map((s) => {
              ...
              if (valor === 0) return null;
              return (
                <li key={s.clave} className="text-xs text-texto-tenue">
