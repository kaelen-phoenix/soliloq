...
export function FotosObra({
  ...
  async function agregar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const ruta = `${creadorId}/obras/${obraId}/${crypto.randomUUID()}.${extension}`;
    ...
    const { data, error: errorInsert } = await supabase
      .from("fotos_obra")
      .insert({ obra_id: obraId, storage_path: ruta, orden: siguienteOrden })
      .select()
      .single();
  ...
  async function eliminar(foto: FotoObra) {
    const supabase = createClient();
